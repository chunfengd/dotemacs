...
(defun my/org-babel-execute:C++ (body params)
  (let* ((image "gcc")
         ...
         (bin-filename "main")
         (host-dir-path (format "~/%s" dirname))
         (host-include-path default-directory)
         (host-src-file-path (format "~/%s/%s" dirname src-filename))
         (host-bin-file-path (format "~/%s/%s" dirname bin-filename))
         (docker-dir-path (format "/%s" dirname))
         (docker-include-path (format "/%s" include-dirname))
         (docker-src-file-path (format "/%s/%s" dirname src-filename))
         (docker-bin-file-path (format "/%s/%s" dirname bin-filename))
...
#+end_src
*** add this to =~/.emacs= if need to use docker
#+begin_src emacs-lisp :tangle no
(add-hook 'org-mode-hook
          (lambda ()
            (fset 'org-babel-execute:C++ 'my/org-babel-execute:C++)))
#+end_src
* rust
#+begin_src emacs-lisp
